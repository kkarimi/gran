...
- `src/plugin-registry.ts`: shipped plugin definitions, ids, defaults, labels, and capabilities
- `src/plugins.ts`: persisted plugin enablement using a generic `enabled` map
- `src/app/plugin-state.ts`: projection helpers and capability checks
- `src/server/routes/plugins.ts`: generic plugin list and toggle routes

The important boundary is this:

1. The registry knows concrete plugin ids.
2. Persistence stores generic enablement by id.
3. App/web/TUI code consume plugin lists and capabilities.
...
Compatibility still matters. The config layer keeps old keys like
`automation-plugin-enabled` and `markdown-viewer-plugin-enabled` working, but it normalises them
into the generic plugin map before the rest of the app sees them.

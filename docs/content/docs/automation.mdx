...
That generates a local `.gran.json`, starter harnesses, starter prompts, and starter automation
...
- macOS: `~/Library/Application Support/gran/automation-rules.json`
- Linux: `~/.config/gran/automation-rules.json`

Override that path with either:

- `--rules /path/to/rules.json`
- `GRANOLA_AUTOMATION_RULES_FILE`
- `automation-rules-file = "/path/to/rules.json"` in `.gran.json`
...
- `OPENROUTER_API_KEY` or `GRAN_OPENROUTER_API_KEY` (`GRANOLA_OPENROUTER_API_KEY` still works)
- `OPENAI_API_KEY` or `GRAN_OPENAI_API_KEY` (`GRANOLA_OPENAI_API_KEY` still works)

For local Codex runs, the toolkit shells out to `codex exec`, so it uses whatever local Codex login
state you already have. You can override the command path with `GRAN_CODEX_COMMAND` (`GRANOLA_CODEX_COMMAND`
still works) or `"codex-command": "..."` in `.gran.json`.
...
Non-secret defaults can live in `.gran.json`:
...
that with `GRAN_PKM_TARGETS_FILE` (`GRANOLA_PKM_TARGETS_FILE` still works) or `"pkm-targets-file": "..."` in `.gran.json`.
  ...
  "targets": [
    {
      "id": "obsidian-team",
      "kind": "obsidian",
      "outputDir": "~/Vaults/Work/Meetings",

...
That generates a local `.granola.toml`, starter harnesses, starter prompts, and starter automation
...
```bash
granola meeting export <meeting-id> --format json > eval-fixtures/customer-sync.json
granola automation evaluate --fixture eval-fixtures/customer-sync.json --harness customer-call
granola automation evaluate --fixture eval-fixtures/customer-sync.json --harness customer-call --provider openrouter --model openai/gpt-5-mini
```
...
`granola automation evaluate` runs the selected harnesses against each case, parses the pipeline

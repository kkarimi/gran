...
that with `GRAN_PKM_TARGETS_FILE` or `"pkm-targets-file": "..."` in `.gran.json`.
...
stable file name. Obsidian targets default to:
...
`docs-folder` targets keep the same idempotent publishing model without Obsidian-specific links,

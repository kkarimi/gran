...
`gran serve` starts a long-lived local Gran 👵🏻 server on one shared app instance.
...
The service persists its discovery metadata in the shared Gran 👵🏻 data directory, so other
surfaces can find it automatically.

## Web

`gran web` now prefers the reusable background service: it will attach to the running service if

...
| P2       | In Progress | M    |              | Server / HTTP Route Modules              | Break the local HTTP server into route modules for auth, meetings, automation, exports, and web assets.                                 | `startGranolaServer()` is still a full router in one function and hard to extend safely.        |
| P2       | Pending     | M    |              | Web / Component Domain Split             | Split `components.tsx` into meeting, review, settings, and home/search component groups.                                                | The component surface is still one warehouse file, which hides page boundaries.                 |
| P2       | Pending     | M    |              | TUI / Workspace Controller Split         | Separate TUI data/control logic from rendering so the workspace is easier to extend and reason about.                                   | `workspace.ts` still mixes loading, input handling, overlays, and rendering in one class.       |

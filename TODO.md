...
| P2       | In Progress | M    |              | TUI / Workspace Controller Split         | Separate TUI data/control logic from rendering so the workspace is easier to extend and reason about.                                   | `workspace.ts` still mixes loading, input handling, overlays, and rendering in one class.       |

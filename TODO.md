...
| P2       | Pending | M    |              | Search / Advanced Quick Open            | Demote “open by meeting id” into advanced search or command palette behaviour instead of promoting it in the main sidebar.                             | Opening by raw id is a niche power-user action, not primary navigation.                               |
| P2       | Pending | M    |              | Diagnostics / Advanced Details          | Move raw runtime and debugging details into a separate diagnostics/settings surface instead of the top-level workspace header.                         | The app needs a clean product UI without losing operator/debug access when needed.                    |
...
Status note:
The browser foundation is calmer now, but it is still not ready to release. The remaining gaps are advanced search, diagnostics separation, sync health/recovery, and cleaner auth/provider setup.
